...
def experiment_paths(experiment_name, experiment_number, dataset_root):
    ...
    paths['timestamps_file'] = os.path.join(paths['experiment_folder'], 'timestamps.txt')
    paths['disparity_folder'] = os.path.join(paths['experiment_folder'], 'disparity_image')
    paths['disparity_file'] = os.path.join(paths['disparity_folder'], '%0.6i.png')
    paths['description'] = os.path.join(dataset_root, 'readme.txt')
    paths['hdf_file'] = os.path.join(paths['experiment_folder'], f'{experiment_name}{experiment_number}_gt.hdf5')
